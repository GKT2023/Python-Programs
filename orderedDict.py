...
for i in range(n):
    inp = input()
    inp = inp.split()
    price = [int(num) for num in inp if num.isdigit()]
    print(price)
    item = " ".join([item for item in inp if item.isalpha()])
    print(item)
    if inp[0] in super_market.keys():
        print("key already exist")
        super_market[item[0:]] += int(price[0])
        # super_market[inp[0]] = int(inp[1]) + int(inp[1])
    else:
        super_market[item[0:]] = int(price[0])

print(super_market)

for key,val in super_market.items():
    print(key, val)
